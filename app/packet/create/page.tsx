...
import {
  ...
  ArrowRight, ExternalLink, ShieldCheck, Search, PlusCircle, History
} from "lucide-react"
...
import MyPacketsHistory from "@/components/packet/my-packets-history"
          ...
          <div className="flex-1 flex flex-col items-center p-6 space-y-6 relative z-10 animate-slide-in">
            <div className="w-full bg-white rounded-[2.5rem] shadow-2xl border border-border overflow-hidden">
              ...
              <div className="bg-muted/20 p-6 grid grid-cols-2 gap-3 border-t border-border">
                 <button onClick={() => router.push("/home")} className="py-4 rounded-2xl bg-white border border-border font-bold text-sm shadow-sm active:scale-95 transition-all flex items-center justify-center gap-2">
                   <Home size={18} /> Home
                 </button>
                 <button onClick={() => handleCopy(createdPacket.shareLink)} className="py-4 rounded-2xl bg-primary text-primary-foreground font-bold text-sm shadow-lg shadow-primary/20 active:scale-95 transition-all flex items-center justify-center gap-2">
    ...
    <div className="min-h-dvh bg-gray-50 flex flex-col max-w-lg mx-auto">
      ...
      <div className="p-4">
        <div className="flex gap-2 p-1 bg-gray-100 rounded-full">
          <button onClick={() => setActiveTab('create')} className={`flex-1 py-2 rounded-full font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'create' ? 'bg-white shadow' : 'text-gray-500'}`}>
            <PlusCircle size={16} /> Create
          </button>
          <button onClick={() => setActiveTab('history')} className={`flex-1 py-2 rounded-full font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'history' ? 'bg-white shadow' : 'text-gray-500'}`}>
            <History size={16} /> History
          </button>
        </div>
      </div>
      
      <div className="flex-1 bg-white" style={{ display: activeTab === 'create' ? 'flex' : 'none', flexDirection: 'column' }}>
        ...
        {showDrawer && selectedTheme && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-50 max-w-lg mx-auto bg-white rounded-t-[3rem] shadow-[0_-20px_60px_rgba(0,0,0,0.2)] border-t border-border h-[90vh]"
            >
              <div className="h-full overflow-y-auto pb-20">
                <div className="w-16 h-1.5 bg-gray-200 rounded-full mx-auto my-4 shadow-inner" />
                <div className="p-6">
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <button onClick={handleCloseDrawer} className="p-2.5 bg-gray-100 rounded-full active:scale-90 transition-all">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="text-center space-y-2 py-6">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-[70%] bg-transparent text-center text-7xl font-black outline-none placeholder:text-gray-200 tracking-tighter"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 gap-4 bg-gray-100/50 p-2 rounded-[2.5rem] border">
                      ...
                      <div className="flex p-1.5 bg-white rounded-2xl border shadow-inner">
                        <button
                          onClick={() => setDistributionType("RANDOM")}
                          className={`flex-1 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                            distributionType === "RANDOM" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground"
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                            distributionType === "EQUAL" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground"
                          }`}
                        >
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 bg-gray-100/50 rounded-2xl border">
                          <div className="flex items-center gap-3">
                              <Users size={20} className="text-primary" />
                              <div>
                              <p className="text-sm font-bold">Private Circle</p>
                              <p className="text-xs text-gray-500">Only selected friends can claim</p>
                              </div>
                          </div>
                          <button 
                              onClick={() => setIsPrivate(!isPrivate)}
                              className={`w-12 h-6 rounded-full transition-all relative ${isPrivate ? 'bg-primary' : 'bg-gray-200'}`}
                          >
                      ...
                      {isPrivate && (
                          ...
                          <div className="relative">
                              ...
                              className="w-full pl-10 pr-4 py-3 bg-gray-100/80 rounded-xl text-sm outline-none focus:ring-1 ring-primary"
                              />
                          </div>
                          <div className="max-h-40 overflow-y-auto space-y-2 pr-2 scrollbar-hide">
                              ...
                              <button 
                                  ...
                                  selectedContacts.find(c => c.id === contact.id) ? 'border-primary bg-primary/5' : 'border-gray-200 bg-white'
                                  }`}
                              >
                                  <div className="flex items-center gap-3">
                                  ...
                                  {selectedContacts.find(c => c.id === contact.id) && <Check size={16} className="text-primary" />}
                              </button>
                              ))}
                          </div>
                    ...
                    <button
                      ...
                      className="w-full py-5 rounded-2xl bg-black text-white font-bold text-lg shadow-xl flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-40 transition-all mt-4 group"
                    >
                      {isLoading ? <Loader2 className="animate-spin" /> : <><span>Blast USDC Packet</span><Gift size={22} className="group-hover:rotate-12 transition-transform" /></>}
                    </button>
                  </div>
                </div>
              </div>
